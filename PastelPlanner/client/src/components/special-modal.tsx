  ...
  const renderContent = (text: string) => {
    try {
      ...
      lines.forEach((line, idx) => {
        ...
        // Headings
        if (/^###\s+/.test(trimmed)) return elements.push(
          <h3 key={idx} className="text-base font-bold mt-3">{trimmed.replace(/^###\s+/, '')}</h3>
        );
        if (/^##\s+/.test(trimmed)) return elements.push(
          <h2 key={idx} className="text-lg font-bold mt-4">{trimmed.replace(/^##\s+/, '')}</h2>
        );
        if (/^#\s+/.test(trimmed)) return elements.push(
          <h1 key={idx} className="text-xl font-bold mt-5">{trimmed.replace(/^#\s+/, '')}</h1>
        );

        // Checklists
        if (/^- \s?/.test(trimmed)) return elements.push(
          <div key={idx}><input type="checkbox" disabled className="mr-2" />{trimmed.replace(/^- \s?\s*/, '')}</div>
        );
        if (/^- x/i.test(trimmed)) return elements.push(
          <div key={idx} className="line-through text-gray-600">
        ...
        if (imgMatches.length) {
          ...
          imgMatches.forEach((m) => {
            ...
            parts.push(
              <img
                ...
                src={m[1]}
                alt="img"
                className="my-2 max-h-60 rounded-xl border border-gray-300"
          ...
          elements.push(<div key={idx}>{parts}</div>);
          return;
        }
      ...
      console.error("Render error in SpecialModal:", err);
      return (
        <p className="text-red-600 text-sm">
          ⚠️ Failed to render content. Please check formatting.
        </p>
      );
    }
  };
